...
import type { CardData, FeaturedProject } from '@/lib/card';
import {
  buildSharedCardUrl,
...
const CURATED_THEME_OPTIONS = [
  ...
  { value: 'velvet-night', labelZh: '丝绒夜幕', labelEn: 'Velvet Night', preview: 'bg-[linear-gradient(135deg,#140f24_0%,#6d28d9_55%,#ec4899_100%)] border-[#6d28d9]' },
  ...
  { value: 'aurora-mint', labelZh: '极光薄荷', labelEn: 'Aurora Mint', preview: 'bg-[linear-gradient(135deg,#f5fffb_0%,#d1fae5_50%,#dbeafe_100%)] border-[#99f6e4]' },
  ...
  { value: 'sunset-paper', labelZh: '日落纸感', labelEn: 'Sunset Paper', preview: 'bg-[linear-gradient(135deg,#fff8f1_0%,#ffedd5_45%,#fce7f3_100%)] border-[#fdba74]' },
] as const;
...
function buildTrustCopyParts({
  ...
  if (isZh) {
    ...
    if (trustTier === 'usage-imported' || trustTier === 'strong-authenticated') {
      return {
        trustLead: `这张卡已经带有可信证明，当前落在 ${rankingSignalLabel}。`,
    ...
    return {
      ...
      trustTail: `${rankingSignalDescription} 后面我会再补截图。`,
    };
  }
  ...
  if (trustTier === 'usage-imported' || trustTier === 'strong-authenticated') {
    return {
      trustLead: `This card already carries proof and currently sits in the ${rankingSignalLabel}.`,
  ...
  return {
    ...
    trustTail: `${rankingSignalDescription} I can add screenshot proof later if I want stronger comparison signals.`,
  };
}
  ...
  if (vibe === 'technical') {
    if (trustTier === 'usage-imported' || trustTier === 'strong-authenticated') {
      return {
        ...
        helper: isZh ? '更适合带截图佐证的高可信表达。' : 'Best for proof-backed, higher-trust sharing.',
    ...
    return {
      ...
      helper: isZh ? '先发也可以，补截图后说服力更高。' : 'Works now, but becomes stronger once screenshot proof is added.',
...
export default function CardEditor({ initialLocale = 'zh' }: CardEditorProps) {
  ...
  const stageBackground = data.theme === 'brand-dark' || data.theme === 'velvet-night' ? '#05070b' : '#eef2ff';
  ...
  const tokenWindowLabel = useMemo(() => getTokenWindowLabel(data.tokenWindow, data.locale), [data.locale, data.tokenWindow]);
  ...
  const rankingAccessGuide = useMemo(() => {
    if (data.trustTier === 'self-reported') {
      return {
        ...
        description: isZh
          ? '现在先展示档位和项目，适合先传播。补一张截图后，就会按当前周期进入正式排名。'
          : 'The card currently leads with tier and project only. Add one screenshot to join official rankings for the selected token window.',
        nextStep: isZh ? '下一步：补一张截图，并确认日期范围与当前统计周期一致。' : 'Next step: add one screenshot and keep the date range aligned with the selected token window.',
      };
    }

    return {
      title: isZh ? '已进入正式排名' : 'Now ranking officially',
      description: isZh
        ? '截图佐证后，这张卡会按当前选择的日 / 周 / 月周期进入排行榜，并显示更明确的排名反馈。'
        : 'With screenshot proof attached, this card joins the leaderboard for the selected day / week / month window and can show clearer rank feedback.',
      nextStep: isZh ? '当前状态已经够用，后续只需要继续更新 token、周期和项目。' : 'You are already ranking. Keep the token number, window, and project info fresh.',
    };
  }, [data.trustTier, isZh]);
  ...
  const fetchRankSummaryWithRetry = useCallback(async (cardId: string): Promise<RankSummary | null> => {
    for (let attempt = 0; attempt < 4; attempt += 1) {
      try {
        const response = await fetch(`/api/rank?id=${cardId}`);
        if (response.ok) {
          const payload = await response.json() as RankSummary;
          if (payload?.globalRank) {
            return payload;
          }
        }
      } catch {
        // Ignore transient sync errors and retry.
      }
  ...
  useEffect(() => {
    ...
    void fetchRankSummaryWithRetry(shareCardId)
      .then((payload) => {
        if (!cancelled) {
  ...
  }, [fetchRankSummaryWithRetry, shareCardId]);

  useEffect(() => {
    if (step !== 3) return;
    setData((prev) => {
      if (prev.proofDateRange?.start || prev.proofDateRange?.end) {
        return prev;
      }
      return {
        ...prev,
        proofDateRange: getCurrentMonthDateRange(),
      };
    });
  }, [step]);
  ...
  }, [data.locale, data.primaryProjectName, data.primaryProjectPitch, data.tokenWindow, data.totalTokens, data.username, isZh]);
  ...
  const applyScreenshotBackedTrust = useCallback(() => {
    if (proofFiles.length === 0) {
      setProofFeedback(isZh ? '请先选择一张截图凭证。' : 'Select at least one screenshot first.');
      return;
    }
    ...
    setData((prev) => ({
      ...
      proofDateRange: prev.proofDateRange?.start || prev.proofDateRange?.end ? prev.proofDateRange : getCurrentMonthDateRange(),
      importedAt: new Date().toISOString(),
    }));
    setProofFeedback(isZh ? '这张卡已升级为“截图佐证”，会按当前周期参与正式排名。' : 'This card is now proof-backed and will rank inside the current token window.');
  ...
  const buildShareCaption = useCallback(() => {
    ...
    return isZh
      ? [
          `${data.username || '我'}${tokenWindowLabel}烧了 ${tokenText} token`,
      ...
      : [
          `${data.username || 'I'} burned ${tokenText} AI tokens ${data.tokenWindow === 'day' ? 'today' : data.tokenWindow === 'week' ? 'this week' : 'this month'}`,
          projectLabel ? `Project: ${projectLabel}` : '',
          projectPitch || activeMetaphor,
          rankingSignalLabel,
          '#TokCard #AIBuilders',
        ].filter(Boolean).join('\n');
  }, [activeMetaphor, data.primaryProjectName, data.primaryProjectPitch, data.tokenWindow, data.totalTokens, data.username, isZh, rankingSignalLabel, sortedAiCaptions, tokenWindowLabel]);
  ...
  const handleExport = useCallback(async () => {
    ...
    try {
      ...
      const nextRankSummary = nextCardId && canParticipateInRanking(cardWithRef.trustTier)
        ? await fetchRankSummaryWithRetry(nextCardId)
        : null;
      const nextExportCard = {
        ...cardWithRef,
        qrcodeUrl: nextShareLink,
        globalRank: nextRankSummary?.globalRank,
        percentile: nextRankSummary?.percentile,
      };

      setShareRankSummary(nextRankSummary);
  ...
  }, [buildShareCaption, data, downloadViaLink, exportIssues, exportRenderId, fetchRankSummaryWithRetry, flashExportNotice, isPreviewReady, isZh, shareCaption, waitForImagesToSettle]);
  ...
  return (
    <div className="min-h-screen text-[#1d1d1f] bg-[#fbfbfd] selection:bg-[#0071e3] selection:text-white">
      ...
      <div className={`max-w-7xl mx-auto px-4 pt-8 ${isMobile ? 'pb-32' : 'pb-8'}`}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 xl:gap-12">
          {/* Left: Form */}
          <div className={`min-w-0 ${showMobileSheet ? 'fixed inset-x-0 bottom-0 z-40 max-h-[78vh] overflow-y-auto rounded-t-[28px] border border-[#dbe4ff] bg-[#fbfbfd]/96 px-4 pt-4 pb-40 shadow-[0_-28px_90px_rgba(15,23,42,0.30)] backdrop-blur-xl' : 'hidden'} lg:block lg:space-y-8 lg:relative lg:max-h-none lg:overflow-visible lg:rounded-none lg:border-0 lg:bg-transparent lg:px-0 lg:pt-0 lg:pb-0 lg:shadow-none`}>
            {showMobileSheet && (
            ...
            <div className="grid gap-4 md:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]">
              ...
              <div>
              <label className="block text-[13px] font-semibold text-[#86868b] mb-2 uppercase tracking-wide">
                {isZh ? `${tokenWindowLabel} Token 用量` : `${tokenWindowLabel} tokens`}
              </label>
              <input
                type="text"
                value={tokenInput}
              ...
              <div className="mt-3">
                <div className="text-[11px] font-semibold uppercase tracking-[0.18em] text-[#94a3b8]">{isZh ? '统计周期' : 'Token window'}</div>
                <div className="mt-2 grid grid-cols-3 gap-2">
                  {(['day', 'week', 'month'] as const).map((window) => (
                    <button
                      key={window}
                      type="button"
                      onClick={() => updateField('tokenWindow', window)}
                      className={`min-h-11 rounded-2xl px-3 text-sm font-medium transition-all ${
                        data.tokenWindow === window
                          ? 'bg-[#0071e3] text-white shadow-md'
                          : 'border border-[#dbe4ff] bg-white text-[#475569] hover:border-[#0071e3]'
                      }`}
                    >
                      {getTokenWindowLabel(window, data.locale)}
                    </button>
                  ))}
                </div>
                <p className="mt-2 text-xs text-[#94a3b8]">
                  {isZh ? '这张卡会和同周期的用户一起比较排名。' : 'This card ranks against builders sharing the same token window.'}
                </p>
              </div>
            ...
            <div>
              ...
              <p className="mt-2 text-xs text-[#86868b]">
                {isZh ? '现在保留 6 个高完成度方向：夜幕、丝绒、柔光、极光、雾面、日落。' : 'There are now 6 polished directions: midnight, velvet, glow, aurora, minimal, and sunset.'}
              </p>
            </div>
                  ...
                  <div className="rounded-2xl border border-[#dbe4ff] bg-white p-4">
                    <div className="flex flex-wrap gap-2">
                      <button
                        ...
                        className="rounded-full bg-[#0071e3] px-4 py-2 text-sm font-medium text-white transition-all hover:opacity-95"
                      >
                        {isZh ? '上传截图凭证' : 'Add screenshot proof'}
                      </button>
                    ...
                    <div className="mt-4 grid gap-4">
                      <div className="rounded-2xl border border-[#dbe4ff] bg-[#f8fbff] p-4">
                        ...
                        <p className="mt-2 text-xs leading-5 text-[#64748b]">{isZh ? '目前只支持截图佐证。截图仅保留在本地，用来确认这张卡可以进入正式排名。' : 'Screenshot proof is the only ranking path for now. Files stay local and are only used to confirm ranking eligibility.'}</p>
                        <button
                          type="button"
                          onClick={() => proofFileInputRef.current?.click()}
                      ...
                      <div className="space-y-3 rounded-2xl border border-[#e2e8f0] bg-white p-4">
                        ...
                        <div className="rounded-2xl border border-[#dbe4ff] bg-[#f8fbff] px-4 py-3 text-xs leading-5 text-[#64748b]">
                          {isZh ? '默认已填当前月。只要截图覆盖的是你这张卡选择的统计周期，就可以直接进入正式排名。' : 'The date range defaults to the current month. As long as the screenshot covers the same token window as this card, it is ready for official ranking.'}
                        </div>
                        <button
                          type="button"
                        ...
                        >
                          {isZh ? '标记为截图佐证并进入排名' : 'Mark as proof-backed and join rankings'}
                        </button>
                      </div>
                    </div>
          ...
          {/* Right: Card Preview */}
          <div className={`lg:sticky lg:top-24 lg:self-start relative z-0 min-w-0 ${isMobile && step === 4 ? 'pb-28' : ''}`} ref={previewContainerRef}>
            <div className="hidden rounded-[24px] border border-[#dbe4ff] bg-white/92 p-5 shadow-sm backdrop-blur mb-6 lg:block">
            ...
            <div className="mb-4 lg:hidden">
              <div className="flex flex-wrap justify-center gap-2">
                <span className="rounded-full border border-[#dbe4ff] bg-white px-3 py-1 text-[11px] font-medium text-[#475569]">{isZh ? 'Token 战绩' : 'Token proof'}</span>
                <span className="rounded-full border border-[#dbe4ff] bg-white px-3 py-1 text-[11px] font-medium text-[#475569]">{isZh ? '主项目' : 'Project'}</span>
                <span className="rounded-full border border-[#dbe4ff] bg-white px-3 py-1 text-[11px] font-medium text-[#475569]">{isZh ? '排名入口' : 'Rank path'}</span>
              </div>
            </div>
            <div className="mb-4 text-center lg:mb-6">
              <div className="text-sm font-medium text-[#86868b]">{isZh ? '实时预览' : 'Live Preview'}</div>
              <div className="mt-2 hidden text-xs leading-5 text-[#94a3b8] lg:block">{isZh ? '先看卡片有没有名片感，再决定是否补充高级选项。' : 'Judge the card as a social identity first, then decide whether it needs extra tuning.'}</div>
              <div className="mt-2 text-xs leading-5 text-[#94a3b8] lg:hidden">{isZh ? '先看卡片本身，再决定还要不要继续调。' : 'Look at the card first, then keep tuning if needed.'}</div>
                ...
                <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                  <div>
                    ...
                    {shareRankSummary ? (
                      <>
                        <div className="mt-3 flex flex-wrap gap-2">
                          <span className="inline-flex items-center rounded-full border border-[#c7ddff] bg-[#eef5ff] px-3 py-1 text-xs font-semibold text-[#0071e3]">
                            {isZh ? `${tokenWindowLabel}全球第 #${shareRankSummary.globalRank} 名` : `${tokenWindowLabel} · Global #${shareRankSummary.globalRank}`}
                          </span>
                      ...
                      <div className="mt-3 text-xs leading-5 text-[#64748b]">
                        {isZh ? '当前还是「用户填写」状态，所以会展示档位信号，但不会进入正式排名。补一张截图后会更强。' : 'This card is still self-reported, so it shows a tier signal but does not enter the official ranking yet. Add one screenshot proof to unlock stronger ranking views.'}
                      </div>
                    )}
                  </div>
                  <a
                    href={`/rank?focus=${shareCardId}&time=${data.tokenWindow}`}
